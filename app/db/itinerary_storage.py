...
def soft_delete_itinerary_for_user(itin_id: str, user_id: Optional[str]) -> tuple[bool, str]:
    """
    ...
    try:
        ...
        if not row.get("success"):
            ...
            if error_code == "forbidden":
                logger.warning(f"User {user_id} attempted to delete itinerary {itin_id} - forbidden")
...
def update_itinerary_meta_for_user(itin_id: str, meta_updates: dict, user_id: Optional[str]) -> tuple[bool, str]:
    """
    Update itinerary meta fields with ownership verification using RPC.

    Args:
        itin_id: The itinerary UUID
        meta_updates: Dict of meta fields to update. Supported keys:
            - title (str)
            - dates (dict)
            - travelers (dict)
            - preferences (dict)
    ...
    try:
        ...
        result = supabase.rpc(
            "rpc_update_itinerary_meta",
            {
                ...
                "p_hotels": meta_updates.get("hotels"),
                "p_mandatory_pois": meta_updates.get("mandatory_pois"),
                "p_ideas": meta_updates.get("ideas"),
            },
        ).execute()
        ...
        if not row.get("success"):
            error_code = row.get("error_code", "error")
            if error_code == "forbidden":
                logger.warning(f"User {user_id} attempted to update itinerary {itin_id} - forbidden")
            return False, error_code
...
def update_itinerary_plan_for_user(itin_id: str, plan: dict, user_id: Optional[str]) -> tuple[bool, str]:
    """
    ...
    try:
        ...
        if not row.get("success"):
            error_code = row.get("error_code", "error")
            if error_code == "forbidden":
                logger.warning(f"User {user_id} attempted to update plan for itinerary {itin_id} - forbidden")
            return False, error_code
...
def load_itinerary_for_user(itin_id: str, user_id: Optional[str]) -> tuple[Optional[dict], str]:
    ...
    try:
        ...
        if access_status == "forbidden":
            logger.warning(f"User {user_id} attempted to access itinerary {itin_id} - forbidden")
        ...
        data = {
            ...
            "meta": {
                ...
                "destinations": row.get("destinations", []) or [],
                "dates": row.get("dates", {}) or {},
                "num_days": (row.get("dates") or {}).get("days") or len(plan.get("days", [])),
