...
class TestDetermineHotelEvents:
    ...
    def test_transition_day_has_both(self, hotel, prev_hotel):
        ...
        events = determine_hotel_events(num_days=2, hotel=hotel, is_first_city=False,
                                        is_last_city=True, prev_city_hotel=prev_hotel)
    ...
    def test_single_day_last_only_checkout(self, hotel, prev_hotel):
        ...
        events = determine_hotel_events(num_days=1, hotel=hotel, is_first_city=False,
                                        is_last_city=True, prev_city_hotel=prev_hotel)
...
class TestDaySpecProperties:
    ...
    def test_has_hotel_event(self):
        day = DaySpec(day_index=0, date=dt.date(2025, 1, 15), start_min=9*60, end_min=20*60,
                      depot_id="h1", hotel_events=[
...
class TestCreateHotelEventNodes:
    ...
    def test_creates_nodes_with_coords(self):
        day_specs = [DaySpec(day_index=0, date=dt.date(2025, 1, 15), start_min=9*60, end_min=20*60,
                             depot_id="h1", hotel_events=[
                                 HotelEvent(HotelEventType.CHECK_IN, "h1", "Hotel", 1.3, 103.8,
                                           (14*60, 16*60), service_time=30)])]
    ...
    def test_stay_not_mandatory(self):
        day_specs = [DaySpec(day_index=0, date=dt.date(2025, 1, 15), start_min=9*60, end_min=20*60,
                             depot_id="h1", hotel_events=[
                                 HotelEvent(HotelEventType.STAY, "h1", "Hotel", 1.3, 103.8,
                                           (0, 24*60), service_time=0)])]
        nodes, _ = create_hotel_event_nodes(day_specs, start_idx=1)
